...
router.get('/shoplist',(req,res)=>{
    sql=`SELECT sid,sh_name,sh_logo,sold_month,distance,star,min_price,is_fengniao,is_grt,is_reciept,is_ad,is_newshop,is_first_dsc,is_dsc,is_manjian,is_brand FROM index_shop`;
    pool.query(sql,[],(err,result)=>{
        if(err) throw err;
        res.send({code:1,msg:result});
    });
